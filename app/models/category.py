...
def get_categories(data, page):
    ...
    listings = Listing.query.filter(Listing.status == True).order_by(Listing.createdAt.desc()).paginate(page,
                                                                                                        current_app.config[
                                                                                                            'POSTS_PER_PAGE'],
                                                                                                        False)
    if list:
        listings = Listing.query.join(
            listing_tags, (listing_tags.c.listing_id == Listing.id)).filter(
            listing_tags.c.category_id.in_(list)).filter(Listing.status == True).order_by(
            Listing.createdAt.desc()).paginate(page, current_app.config['POSTS_PER_PAGE'], False)
    return listings
